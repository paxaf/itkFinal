...
import (
	...
	loggermocks "github.com/paxaf/itkFinal/gw-notification/internal/mocks/logger"
	kafkamocks "github.com/paxaf/itkFinal/gw-notification/internal/mocks/transport/kafka"
	usecasemocks "github.com/paxaf/itkFinal/gw-notification/internal/mocks/usecase"
...
type ConsumerSuite struct {
	...
	ctx      context.Context
	handler  *usecasemocks.LargeOperationHandlerMock
...
func (s *ConsumerSuite) SetupTest() {
	...
	s.handler = usecasemocks.NewLargeOperationHandlerMock(s.T())
...
func (s *ConsumerSuite) TestHandleMessagesDecodesValidMessagesAndSkipsBrokenJSON() {
	...
	s.handler.EXPECT().HandleLargeOperations(s.ctx, []domain.LargeOperationEvent{event1, event2}).Return(usecase.HandleLargeOperationsResult{
		Received: 2,
		Valid:    2,
		Accepted: 2,
	}, nil).Once()
...
func (s *ConsumerSuite) TestHandleMessagesLogsResultAndDecodeFailures() {
	...
	s.handler.EXPECT().HandleLargeOperations(s.ctx, []domain.LargeOperationEvent{event}).Return(usecase.HandleLargeOperationsResult{
		Received: 1,
		Valid:    1,
		Accepted: 1,
	}, nil).Once()
	log.EXPECT().Info("large operation batch handled", mock.MatchedBy(func(fields map[string]interface{}) bool {
...
func (s *ConsumerSuite) TestHandleMessagesReturnsHandlerError() {
	event := validEvent("event-1")
	s.handler.EXPECT().HandleLargeOperations(s.ctx, []domain.LargeOperationEvent{event}).Return(usecase.HandleLargeOperationsResult{}, errHandler).Once()
