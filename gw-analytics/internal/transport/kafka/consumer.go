...
type Consumer struct {
	reader    Reader
	handler   usecase.OperationHandler
...
func New(cfg config.Kafka, handler usecase.OperationHandler, log logger.Interface) *Consumer {
...
func (c *Consumer) handleMessages(ctx context.Context, messages []kafkago.Message) error {
	events := make([]domain.OperationEvent, 0, len(messages))
	decodeFailed := 0
	...
	if len(events) == 0 {
		c.logInfo("operation batch skipped", map[string]interface{}{
	...
	result, err := c.handler.HandleOperations(ctx, events)
	if err != nil {
		return fmt.Errorf("handle operation events: %w", err)
	}

	c.logInfo("operation batch handled", map[string]interface{}{
		"messages":      len(messages),
		"decoded":       len(events),
		"decode_failed": decodeFailed,
